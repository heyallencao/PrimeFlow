...
After this skill completes, update `.keystone/state.json`:

```bash
./keystone state set current_stage "[stage-name]"
./keystone state set last_skill "[skill-name]"
# Add skill-specific fields as needed:
# ./keystone state set verify_result "pass"
# ./keystone state set risk_level "medium"
```
...
Emit one event to `.keystone/telemetry/events/YYYY-MM.jsonl`:

```bash
echo "{\"skill\":\"[skill-name]\",\"ts\":\"$(date -u +%Y-%m-%dT%H:%M:%SZ)\",\"decision\":\"[decision-label]\",\"confidence\":[0.0-1.0]}" \
  >> .keystone/telemetry/events/$(date +%Y-%m).jsonl
```
...
- [ ] State has been updated via `./keystone state set`
