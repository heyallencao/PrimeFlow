...
> 对标 gstack/superpowers 的 README 质量，重写 Keystone README 的第一屏到第三屏

**更新说明**：已修正技能数量（18，非19），移除尚未落地的 browse/security，
安装命令改为真实命令 `./keystone install`。
...
Keystone is built around a different contract:
...
superpowers teaches your agent **how** to code. gstack gives your agent **roles** to play. Keystone gives your agent a **conscience**.
...
- **State integration**: reads and writes a shared `.keystone/state.json`
...
After install, restart Claude Code and run `/ks-help` to see all available skills.

**Supported agents:**
- Claude Code → `/ks-[skill-name]` slash commands
- Codex → `ks-[skill-name]` skill directories (auto-scanned)
- Gemini → skill bundle at `~/.gemini/skills/Keystone`

Want to install to a specific agent target only?
```bash
./keystone install --agent claude
./keystone install --agents claude,codex
./keystone install --dry-run --agent gemini   # preview without writing
```
...
Keystone is built on one observation: **the biggest risk in AI-assisted coding isn't slow output — it's false confidence**.
...
This is why every skill in Keystone has an honest exit condition. This is why `verify` can only return `pass` with fresh test results, not code inspection. This is why `release` must disclose what was skipped, not just what was done.
...
*If Keystone changes how you think about AI agents, consider [sharing it](https://github.com/heyallencao/Keystone) with your team.*
...
2. **对比表格**：让人快速理解差异化，加了"Multi-agent support"行，这是 Keystone 的额外优势
...
Keystone is a skill system for Claude Code that enforces a simple rule:
