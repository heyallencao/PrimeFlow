...
- `.agents/skills/`：18个 Codex 预生成技能目录（pf-* 格式）
- `primeflow.manifest.json`：完整的17技能清单 + 三端配置
...
| 维度 | PrimeFlow | superpowers | gstack |
|------|-----------|-------------|--------|
...
PrimeFlow 当前开场没有一句让工程师停下来继续读的钩子。
...
PrimeFlow 的 `qa` 技能提到了 Playwright，但没有独立的 `browse` 技能作为展示层。
...
PrimeFlow 的 `review` 有 security-reviewer persona，但不够独立和显眼——无法作为"安全审计"来单独推广。

#### 差距5：没有 `retro` 技能（这实际上是 PrimeFlow 最大的潜力差异化点）

gstack 的 `/retro` 是手动填写的复盘。

PrimeFlow **有 Telemetry 系统**，可以做数据驱动的自动复盘——这是 gstack 做不到的。这个技能一旦落地，就是明显优于 gstack 的差异化点。
...
| **PrimeFlow 现状** | 只有 GitHub repo + 尚未发布 |
...
**同步**：`.agents/skills/pf-*/SKILL.md` 也需要同步英文化（共18个）
...
## How It's Different
[对比表格：PrimeFlow vs superpowers vs gstack]
...
- [ ] `node install.mjs` 或 `./primeflow install` 在全新环境下是否真的能跑通
- [ ] 安装完成后有没有打印 "What's next" 提示
- [ ] `primeflow doctor` 输出是否清晰友好（✅ / ❌ 格式）
...
**意义**：截图 = 证据，和 PrimeFlow "honest exit" 哲学完全一致
...
#### 2.3 `retro` 技能（PrimeFlow 独有的差异化）

这是 PrimeFlow 唯一能比 gstack 更强的技能：**基于 Telemetry 数据自动生成**，而不是手动填写。
...
What it generates (from .primeflow/telemetry/events/*.jsonl):
...
- PrimeFlow: the data fills it in for you
```
...
- *"对比 gstack 和 superpowers：PrimeFlow 的 State 驱动到底解决了什么"*
...
达到以下标准，PrimeFlow 才算"卓越"而不只是"好用"：

- [ ] **安装体验**：全新工程师 5 分钟内从 0 到用上第一个技能
- [ ] **内容质量**：所有 SKILL.md 可以直接给英文母语工程师看，他们能理解并感到有深度
- [ ] **差异化清晰**：任何人看了 README 后，30秒内能说出 PrimeFlow 和 gstack/superpowers 有什么不同
