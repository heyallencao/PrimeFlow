...
| 没有 manifest | `primeflow.manifest.json` **已有完整17技能配置** |
| 没有 Codex 支持 | `.agents/skills/` **已预生成18个 pf-* 目录** |

---

## 真实竞争力位置

PrimeFlow 实际上**比之前分析更成熟**。真正的差距比想象中要少，但每个差距的影响很大。
...
**需要改的文件数量**：18 个 SKILL.md + 18 个 `.agents/skills/pf-*/SKILL.md` = 36 个文件
...
| **PrimeFlow 现状** | 技术架构描述 | 平淡，无冲动点击 |

**缺少的要素**：
1. 一句让人停下来的 tagline
2. 明确的差异化对比表格（PrimeFlow vs superpowers vs gstack 并列）
3. Terminal GIF（视觉证明）
...
| **PrimeFlow** | qa 提到 Playwright，无独立展示层 | 没有视觉可信度 |
...
| **PrimeFlow** | review 里有 security-reviewer persona（条件触发），不够独立显眼 |
...
| **PrimeFlow** | 有 Telemetry JSONL 数据，但没有 retro 技能 |

**为什么这是机会**：PrimeFlow 是同类产品中**唯一**有 Telemetry 系统的。基于真实决策数据自动生成复盘，比 gstack 手动填写的更有可信度。这是一个可以公开说"我们比 gstack 更强"的点。
...
| **PrimeFlow** | 只有 GitHub repo，尚未对外发布 |
...
这些是 PrimeFlow **真正独有**的竞争优势，在优化过程中必须保留并强化：

### 优势1：状态驱动路由（独一无二）

```
.primeflow/state.json — 56字段
...
```
.primeflow/telemetry/events/YYYY-MM.jsonl
...
```
Claude Code → /pf-* 命令
Codex → .agents/skills/pf-* 预生成目录
Gemini → ~/.gemini/skills/PrimeFlow bundle
```
...
```
PrimeFlow 不是"更多功能的 gstack"
PrimeFlow 不是"更完整的 superpowers"

PrimeFlow 是一套不同维度的产品：
- superpowers 解决"怎么写好代码"
- gstack 解决"谁来做不同的事"
- PrimeFlow 解决"什么时候可以说做完了"
```
