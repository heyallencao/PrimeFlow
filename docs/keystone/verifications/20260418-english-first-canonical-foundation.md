...
| Core runtime skills use canonical English | `ks-help`, `ks-orchestrate`, `ks-writing-plan`, `ks-verify`, and `ks-review` are covered by the English foundation check | covered by `npm run test:english-foundation` | PASS |

### Claims
- The repository now has an explicit English-first language policy for canonical surfaces.
- The root onboarding path no longer depends on Chinese to explain what Keystone is or how to start.
