...
|---|---|---|---|
| Public install docs explain package status honestly | README and installation docs should say PrimeFlow is open source but not yet an npm package, and explain why `package.json` is still `private` | wording added to `README.md` and `docs/installation.md` | PASS |
...
### Claims
- PrimeFlow now states clearly that the repository is open source while npm publishing is still out of scope.
