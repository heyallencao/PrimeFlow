...
| Manifest and repo checks align to source, not `.agents` wrappers | manifest summaries should be English and the English foundation gate should validate source `entry` files | `primeflow.manifest.json` was rewritten in English and `scripts/check-english-foundation.mjs` now targets source skill paths | PASS |
...
### Claims
- PrimeFlow's actual canonical skill source is now English-first across the real runtime `SKILL.md` files.
