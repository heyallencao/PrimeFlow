...
function resolveProjectRoot() {
  ...
  if (process.env.KEYSTONE_PROJECT_ROOT) {
    return { root: path.resolve(process.env.KEYSTONE_PROJECT_ROOT), source: "env" };
  }
...
const keystoneRoot = path.join(projectRoot, ".keystone");
const statePath = path.join(keystoneRoot, "state.json");
const telemetryDir = path.join(keystoneRoot, "telemetry", "events");
const handoffRoot = path.join(keystoneRoot, "handoff");
const command = argv[0];
const cliName = scriptPath.endsWith("keystone.mjs") ? "./keystone" : "keystone";
...
function manifestPathFor(sourceRoot) {
  return path.join(sourceRoot, "keystone.manifest.json");
}
...
function ensureProjectRoot() {
  const hasExplicitProjectRoot = Boolean(projectRootArg || process.env.KEYSTONE_PROJECT_ROOT);
  if (!hasExplicitProjectRoot && isWithinPath(cwd, packageRoot)) {
    fail(
      `Do not run Keystone from the skill package directory.\n` +
...
function parseDoctorArgs(args) {
  const options = {
    home: process.env.KEYSTONE_HOME ? path.resolve(process.env.KEYSTONE_HOME) : homeDir()
  };
...
function resolveInstallAgents(options, manifest) {
  ...
  if (detectedAgents.length === 0) {
    ...
    say("");
    say("Keystone works with Claude Code, Codex CLI, or Gemini CLI.");
...
function writeGeneratedCodexSkillDocs(manifest, outputRoot, runtimeRoot, force = true) {
  ...
  const cliDefaultRef = `${shellRuntimeRoot}/keystone`;
  const cliCommandRef = `"${shellRuntimeRoot}/keystone"`;
...
function resolveGeneratedCodexSkillContent(
  ...
  runtimeRootRef = "~/.keystone/runtime/Keystone"
) {
  ...
  return rewriteInstalledSkillReferences(sourceContent, runtimeRootRef)
    .replaceAll('_KS_CLI="${KEYSTONE_CLI:-./keystone}"', `_KS_CLI="\${KEYSTONE_CLI:-${cliDefaultRef}}"`)
    .replaceAll("./keystone", cliCommandRef);
}
...
function loadInstalledManifest(installRoot) {
  const manifestPath = path.join(installRoot, "keystone.manifest.json");
...
function buildAgentInstallPlan(options, manifest, agent, existingManifest) {
  ...
  const installedCliPath = path.join(runtimeRootForSkills, "keystone");
...
function invocationGuidanceForAgent(manifest, agent) {
  ...
  return "Load the installed Keystone skill bundle and call the needed skill by its explicit name, starting from orchestrate.";
}
  ...
  try {
    for (const operation of operations) {
      ...
      const backupPath = pathExists(operation.target)
        ? `${operation.target}.keystone-backup-${process.pid}-${backupCounter += 1}`
        : null;
      if (backupPath) {
...
function installCommand(args) {
  ...
  say(`- Installed public ks-* skills: ${manifest.skills.map((skill) => path.join(options.home, ".agents", "skills", skill.alias)).join(", ")}`);
  for (const plan of plans) {
    if (plan.mountRuntime) {
  ...
  if (primaryAgent === "claude") {
    ...
    say("  Next step:");
    say("    1. Restart Codex  (so it rescans installed Keystone skills)");
    say("    2. Paste one of these to start:");
    say("");
    say("  Don't know where to start:");
    say("    /ks-help");
    ...
    say("    /ks-orchestrate Determine which entry mode this task should use.");
    say("");
    say("  Have a bug:");
    say("    /ks-bug-triage Classify whether this issue is a spec gap or an implementation bug.");
  } else if (primaryAgent === "gemini") {
    say("  Next step:");
    say("    1. Restart Gemini  (so it rescans installed Keystone skills)");
    say("    2. Paste one of these to start:");
    say("");
    say("  Don't know where to start:");
    say("    /ks-help");
    ...
    say("  Restart your agent and invoke /ks-help.");
  }
...
function parseDistArgs(args) {
  ...
  const options = {
    forceClean: false,
    output: path.join(packageRoot, "dist", "release", "Keystone")
  };
...
function parseGenArgs(args) {
  ...
  const options = {
    ...
    runtimeRoot: "~/.keystone/runtime/Keystone",
    force: false
  };
...
function writeState(nextState) {
  ...
  fsyncPathIfPossible(keystoneRoot);
}
...
function doctor(args = []) {
  ...
  say("Next steps:");
  if (missingCore.length > 0) {
    say("- Install the missing core tools, then rerun `./keystone doctor`.");
    return;
  }

  if (detectedAgents.length === 1) {
    say(`- Run \`./keystone install\` to install Keystone into ${detectedAgents[0]}.`);
  } else if (detectedAgents.length > 1) {
    say(`- Multiple agent targets detected. Choose one explicitly with \`./keystone install --agent <${detectedAgents.join("|")}>\`.`);
    say(`- Or install to several at once with \`./keystone install --agents ${detectedAgents.join(",")}\`.`);
  } else {
    say(`- No supported agent target markers were found under ${options.home}.`);
    say("- Install with an explicit agent target, for example: `./keystone install --agent claude`.");
  }

  say("- After install, start with `help` if you are unsure which Keystone skill to use.");
}
...
function stateCommand(args) {
  ...
  if (subcommand === "init") {
    ...
    // Create first-run flag files so ks-help ritual does not need manual bash
    ensureDir(keystoneRoot);
    const flagFiles = [".first-run", ".telemetry-consent", ".proactive-consent"];
    for (const flag of flagFiles) {
      const flagPath = path.join(keystoneRoot, flag);
...
function scaffoldCatalog() {
  return {
    plan: {
      source: "plan.md",
      defaultOutput: path.join(projectRoot, "docs", "keystone", "plans", `${dateStamp()}-plan.md`)
    },
    "test-contract": {
      source: "test-contract.md",
      defaultOutput: path.join(projectRoot, "docs", "keystone", "tests", `${dateStamp()}-test-contract.md`)
    },
    "review-report": {
      source: "review-report.md",
      defaultOutput: path.join(projectRoot, "docs", "keystone", "reviews", `${dateStamp()}-review-report.md`)
    },
    "release-statement": {
      source: "release-statement.md",
      defaultOutput: path.join(projectRoot, "docs", "keystone", "releases", `${dateStamp()}-release-statement.md`)
    },
    "incident-report": {
      source: "incident-report.md",
      defaultOutput: path.join(projectRoot, "docs", "keystone", "incidents", `${dateStamp()}-incident-report.md`)
    }
  };
}
...
const profilePath = path.join(keystoneRoot, "developer-profile.json");
...
function knowledgeCommand(args) {
  ...
  if (subcommand === "search") {
    ...
    if (queryWords.length === 0) {
      fail("Usage: keystone knowledge search <keyword1> [keyword2] ...");
    }
    const files = findKnowledgeFiles(solutionsRoot);
    if (files.length === 0) {
      say("No knowledge artifacts found in docs/solutions/");
