...
[![CI](https://github.com/heyallencao/PrimeFlow/actions/workflows/ci.yml/badge.svg)](https://github.com/heyallencao/PrimeFlow/actions/workflows/ci.yml)

**[primeflow.pages.dev](https://primeflow.pages.dev)** - docs and demos
...
After installation, restart your agent and start with `/pf-help`.
...
PrimeFlow is a workflow skill bundle for Claude, Codex, and Gemini.
...
You ask for a feature, the agent confidently returns code, and the conversation moves on before anyone proves that tests passed, edge cases were handled, or the behavior was actually verified. PrimeFlow is built around a different contract:
...
PrimeFlow is for teams and solo builders who already trust agents to produce output, but do not trust them to police themselves.
...
PrimeFlow lives in the same problem space as tools like `superpowers` and `gstack`, but it optimizes for a different layer.

| Project | Strongest layer | What PrimeFlow adds |
|---|---|---|
...
| **PrimeFlow** | entry-mode routing, decision contracts, honest exit, shared state, handoff, telemetry | more like a workflow kernel than a loose prompt collection |
...
- **Shared state**: workflow context is stored in `.primeflow/state.json`
...
```text
/pf-orchestrate -> /pf-roundtable -> /pf-writing-plan -> /pf-test-first -> /pf-implement -> /pf-verify -> /pf-review -> /pf-ship -> /pf-release
```

**You already know what to change**

```text
/pf-writing-plan -> /pf-test-first -> /pf-implement -> /pf-verify -> /pf-review
```

**Bug reported, root cause unknown**

```text
/pf-bug-triage -> /pf-diagnose -> /pf-implement -> /pf-verify -> /pf-ship
```
...
PrimeFlow is open source, but it is not published as an npm package today. The supported install paths are the repository install flow and the remote install script above. `package.json` remains intentionally `private` until npm publishing becomes a real supported release path.
...
cd primeflow
./primeflow install --agents claude,codex,gemini
./primeflow doctor
```

### 2. Restart your agent

PrimeFlow installs public `pf-*` skills into `~/.agents/skills` and the shared runtime into `~/.primeflow/runtime/PrimeFlow`.
...
```text
/pf-verify Collect fresh evidence for the current change before claiming it is ready for review.
```
...
- [PrimeFlow for Codex](docs/README.codex.md) - Codex-specific install and usage guide
...
PrimeFlow is now **English-first across canonical surfaces**:
